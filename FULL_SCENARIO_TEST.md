...
**测试步骤**：
1. 输入：`分析overall_cpu_utilization100数据`
2. 输入：`这个CPU使用率看起来有点高啊`
3. 输入：`user模式和system模式哪个更异常？`
4. 输入：`这个峰值是什么时候出现的？`
5. 输入：`如果CPU使用率继续这么高，会有什么影响？`
6. 输入：`有什么优化建议吗？`
7. 输入：`这个服务器现在怎么样？`
8. 验证：AI在整个对话中保持上下文连贯性，不重复获取数据
...
### 场景13：CPU性能与业务影响关联分析
**测试步骤**：
1. 输入：`获取overall_cpu_utilization100数据`
2. 输入：`这个CPU使用率和系统响应时间有关系吗？`
3. 输入：`如果CPU使用率超过80%，会影响用户体验吗？`
4. 输入：`从运维角度，需要扩容吗？`
5. 输入：`扩容的成本和收益如何？`
6. 验证：AI进行CPU性能与业务影响的关联分析

### 场景14：相似查询数据混淆测试
**测试步骤**：
1. 输入：`分析dogecoin_panel_2数据`
2. 输入：`再分析dogecoin_panel_7数据`
3. 输入：`这两个面板的数据有什么不同？`
4. 输入：`为什么dogecoin_panel_7的分析结果和dogecoin_panel_2一样？`
5. 验证：AI正确区分不同查询，不混淆数据
...
- **CPU使用率数据**: 
  ...
  - **查询**: node_cpu_seconds_total (CPU使用率监控)
  - **实例**: faro-shop-control-plane
  - **模式**: user/system/iowait等CPU使用模式
  - **时间范围**: 实时监控数据
  - **数据点**: 778个数据点
  - **面板**: CPU Utilization Details (Cores)
