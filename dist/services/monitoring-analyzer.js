...
function createDataSummary(data) {
    ...
    if (Array.isArray(data)) {
                ...
                if (firstItem.value !== undefined || firstItem.metric || firstItem.timestamp || firstItem.values) {
                    ...
                    const values = data.flatMap((item) => {
                        if (item.values && Array.isArray(item.values)) {
                            return item.values.map((v) => parseFloat(v[1] || v.value || 0));
                        }
        ...
        if (hasLargeArrays || keys.length > 20) {
            ...
            // Grafana常见字段优先级
            const grafanaImportantKeys = keys.filter(key => key.includes('result') ||
                key.includes('data') ||
                key.includes('series') ||
                key.includes('value') ||
                key.includes('metric') ||
                ...
                if (Array.isArray(value)) {
                    if (value.length > 50) {
                        ...
                        if (value[0] && typeof value[0] === 'object') {
                            const item = value[0];
                            if (item.values && Array.isArray(item.values)) {
                                const allValues = value.flatMap((v) => v.values?.map((val) => parseFloat(val[1] || 0)) || []).filter((v) => !isNaN(v));
 ...
 */
export function formatDataForClientAI(prompt, data, config) {
    // 获取数据处理配置
