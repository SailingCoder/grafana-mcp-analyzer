 ...
 * 存储请求信息
 */
export async function storeRequestInfo(
  sessionId: string, 
  requestId: string, 
 ...
 * 列出会话中的所有请求
 */
export async function listSessionRequests(sessionId: string): Promise<Array<Record<string, any>>> {
 ...
 * 删除请求
 */
export async function deleteRequest(sessionId: string, requestId: string): Promise<boolean> {
  const filePath = path.join(SESSIONS_DIR, sessionId, 'requests', `${requestId}.json`);
